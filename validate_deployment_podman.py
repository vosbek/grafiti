#!/usr/bin/env python3
"""
CodeAnalysis MultiAgent MVP - Podman-Compatible Deployment Validation Script

This is a modified version of validate_deployment.py specifically for podman compatibility.
...
class DeploymentValidator:
    """Comprehensive deployment validation with automated fixes for podman"""
    ...
    def __init__(self, comprehensive: bool = False, fix_issues: bool = False):
        ...
        print(f"{Color.CYAN}{Color.BOLD}🔍 CodeAnalysis Deployment Validation (Podman){Color.END}")
        print(f"{Color.WHITE}Mode: {'Comprehensive' if comprehensive else 'Standard'} | Fix Issues: {fix_issues} | Container Runtime: {self.container_runtime}{Color.END}\n")
    ...
    def validate_system_requirements(self) -> bool:
        ...
        # Podman availability (instead of Docker)
        success, stdout, stderr = self._run_command([self.container_runtime, '--version'])
        if success:
            self._add_result(ValidationResult(
                "podman_available", True, f"Podman available: {stdout.strip()}",
                {"version": stdout.strip()}
            ))
        else:
            self._add_result(ValidationResult(
                "podman_available", False, f"Podman not found: {stderr}",
                {"error": stderr}
            ))
            all_passed = False
        
        # Podman-compose availability
        success, stdout, stderr = self._run_command([self.compose_command, '--version'])
        if success:
            self._add_result(ValidationResult(
                "podman_compose_available", True, f"Podman-compose available: {stdout.strip()}",
                {"version": stdout.strip()}
            ))
        else:
            self._add_warning("podman_compose", f"Podman-compose not found: {stderr}")
    ...
    def validate_database_connection(self) -> bool:
        """Validate Neo4j database connection using podman"""
        self._print_test("Database Connection")
        
        all_passed = True
        
        # Check if Podman is available
        podman_available, stdout, stderr = self._run_command([self.container_runtime, '--version'])
        if podman_available:
            self._add_result(ValidationResult(
                "podman_available", True, f"Podman available: {stdout.strip()}",
                {"version": stdout.strip()}
            ...
            success, stdout, stderr = self._run_command([self.container_runtime, 'ps', '--format', '{{.Names}}'])
            if success and 'neo4j-codeanalysis' in stdout:
                ...
                if self.fix_issues:
                    print(f"{Color.YELLOW}  🔧 Starting Neo4j container with podman...{Color.END}")
                    neo4j_command = [
                    ...
                    if success:
                        print(f"{Color.GREEN}  ✅ Neo4j container started with podman{Color.END}")
                        time.sleep(10)  # Wait for Neo4j to be ready
                    else:
                        print(f"{Color.RED}  ❌ Failed to start Neo4j with podman: {stderr}{Color.END}")
        else:
            self._add_result(ValidationResult(
                "podman_available", False, f"Podman not available: {stderr}",
                {"error": stderr}
            ))
            self._add_warning("database", "Neo4j requires Podman or manual installation")
    ...
    def run_validation(self) -> bool:
        ...
        # Print summary
        print(f"{Color.CYAN}{Color.BOLD}📊 Validation Summary (Podman){Color.END}")
        ...
        if overall_success:
            ...
            print(f"{Color.WHITE}Your CodeAnalysis deployment with podman is ready to use.{Color.END}")
        else:
        ...
        if overall_success:
            ...
            print(f"{Color.WHITE}1. Backend: {Color.CYAN}./start_backend_podman.sh{Color.END} {Color.WHITE}or{Color.END} {Color.CYAN}start_backend_podman.bat{Color.END}")
            print(f"{Color.WHITE}2. Frontend: {Color.CYAN}./start_frontend_podman.sh{Color.END} {Color.WHITE}or{Color.END} {Color.CYAN}start_frontend_podman.bat{Color.END}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='CodeAnalysis Deployment Validation (Podman)')
