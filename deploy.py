#!/usr/bin/env python3
"""
CodeAnalysis MultiAgent MVP - Automated Deployment Script

This script automates the complete deployment process across different platforms
with all verified fixes and optimizations applied.

Usage:
    python deploy.py --platform auto --mode production
    python deploy.py --platform windows --mode development --gpu-enabled
    python deploy.py --platform linux --mode production --bedrock
"""
...
class DeploymentManager:
    """Manages the complete deployment process with verified fixes"""
    ...
    def __init__(self, target_platform: str, mode: str, options: Dict):
        ...
        print(f"{Color.CYAN}{Color.BOLD}🚀 CodeAnalysis MultiAgent MVP Deployment{Color.END}")
        print(f"{Color.WHITE}Platform: {self.platform} | Mode: {mode} | Options: {options}{Color.END}\n")
    ...
    def setup_database(self) -> bool:
        ...
        self._print_step("5/10", "Setting up Neo4j database")
        
        # Check if Docker is available
        success, stdout, stderr = self._run_command(['docker', '--version'], check=False)
        if success:
            self._print_success("Docker detected - setting up Neo4j container")
            
            # Check if Neo4j container already exists
            success, stdout, stderr = self._run_command(['docker', 'ps', '-a', '--format', '{{.Names}}'], check=False)
            if 'neo4j-codeanalysis' in stdout:
                self._print_success("Neo4j container already exists - starting if stopped")
                self._run_command(['docker', 'start', 'neo4j-codeanalysis'], check=False)
                return True
            ...
            if success:
                self._print_success("Neo4j container created and started")
                # Wait for Neo4j to be ready
                time.sleep(10)
                return True
            else:
                self._print_error(f"Failed to create Neo4j container: {stderr}")
                return False
        else:
            self._print_warning("Docker not available - please install Neo4j manually")
            self._print_warning("See DEPLOYMENT_GUIDE.md for manual Neo4j installation instructions")
            return True  # Continue deployment without Docker
...
"""
    ...
    def deploy(self) -> bool:
        ...
        print(f"\n{Color.GREEN}{Color.BOLD}🎉 Deployment completed successfully!{Color.END}")
        ...
        if self.options.get('bedrock', False):
            ...
            if self.platform == 'windows':
                print(f"   {Color.CYAN}start_bedrock_proxy.bat{Color.END}")
            else:
                print(f"   {Color.CYAN}./start_bedrock_proxy.sh{Color.END}")
        
        print(f"{Color.WHITE}{'2' if self.options.get('bedrock') else '1'}. Start backend:{Color.END}")
        if self.platform == 'windows':
            print(f"   {Color.CYAN}start_backend.bat{Color.END}")
        else:
            print(f"   {Color.CYAN}./start_backend.sh{Color.END}")
        
        print(f"{Color.WHITE}{'3' if self.options.get('bedrock') else '2'}. Start frontend:{Color.END}")
        if self.platform == 'windows':
            print(f"   {Color.CYAN}start_frontend.bat{Color.END}")
        else:
            print(f"   {Color.CYAN}./start_frontend.sh{Color.END}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='CodeAnalysis MultiAgent MVP Deployment Script')
    ...
    try:
        ...
        if success:
            print(f"\n{Color.GREEN}✅ Deployment completed successfully!{Color.END}")
            sys.exit(0)
        else:
            print(f"\n{Color.RED}❌ Deployment failed. Check the output above for details.{Color.END}")
            sys.exit(1)
